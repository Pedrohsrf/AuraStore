...
async function listarClientes() {
    ...
    const response =
        await fetch("http://localhost:8081/clientes");
    ...
    const response = await fetch(
        "http://localhost:8080/vendas",
        {
            ...
            body: JSON.stringify({
                ...
                produtoId:
                    Number(document.getElementById("produtoId").value),
                quantidade:
                    Number(document.getElementById("quantidade").value),
                valorUnitario:
                    Number(document.getElementById("valorUnitario").value)
            })
        }
    );
...
async function listarVendas() {
    ...
    const response =
        await fetch("http://localhost:8080/vendas");
...
async function buscarVendasCliente() {
    ...
    const response =
        await fetch(
            `http://localhost:8081/clientes/${id}/vendas`
        );
...
async function buscarClienteDaVenda() {
    ...
    const response =
        await fetch(
            `http://localhost:8080/vendas/${vendaId}/cliente`
        );
